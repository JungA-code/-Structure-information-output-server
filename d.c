...
void SHM_Create();
void SHM_Update(int);
ssize_t _dwrite( int fd, const void *buff, size_t nbyte, char *file, int line ); // DH, debug function
#define dwrite(fd, buff, nbyte) _dwrite(fd, buff, nbyte, __FILE__, __LINE__)
...
void *do_send(int arg, char *Data) {
   ...
   printf("%d\n", n);
   //printf("%x\n",n);
   dwrite(c_socket, &n, sizeof(n));
   dwrite(c_socket, Data, strlen(Data));
   EraseSpace(Data);
}
